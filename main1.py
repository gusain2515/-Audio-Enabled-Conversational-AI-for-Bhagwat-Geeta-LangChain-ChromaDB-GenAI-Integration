...
# For audio recording; install with: pip install streamlit-audio-recorder
import  audio_recorder_streamlit as recorder

# --- Your existing imports for Langchain, ChromaDB, and LLM ---
...
os.environ["GOOGLE_API_KEY"] = "AIzaSyBSKEZlXI4MXM3jwTGBTLeJ695-cRbXD9s"
...
# -------------------------------
# 2. Custom Loader for Bhagwat Geeta
# -------------------------------
def custom_load_text(file_path):
    """Load a text file using UTF-8 encoding and ignoring errors."""
    with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
        text = f.read()
    return [Document(page_content=text)]

# -------------------------------
# 3. Build or Load the ChromaDB Index from Bhagwat Geeta
# -------------------------------
persist_directory = "chromadb_index"
text_file = "geeta_txt.txt"  # Ensure your file is named accordingly
...
def retrieve_context(query):
    """Retrieve relevant Bhagwat Geeta context for the given query."""
...
def llm_query(history, context, question):
    """Query the LLM using a prompt that includes conversation history and context."""
...
# -------------------------------
# 6. Streamlit App Layout & Conversation Handling with Audio Input
# -------------------------------
...
# Audio recorder component from streamlit_audio_recorder
...
# Provide an alternative text input
user_query = st.text_input("Or type your question:")

# Use the transcribed text if available, otherwise use typed input
if transcribed_text:
